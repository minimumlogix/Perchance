(function() {
    ...
    // Base URL resolver for dynamic assets/stylesheets
    function getBaseUrl() {
    ...
    window.setSheetStyleLayout = function(styleName) {
        if (!window.sheetsState) {
            window.sheetsState = {
                styleLayout: styleName,
                theme: "emerald",
                identity: {},
                physical: {},
                psychology: {},
            ...
            window.sheetsState.styleLayout = styleName;
        }
        
        window.applySheetStyleLayout(styleName);
        window.autoSaveSheet();
        renderSheetContent();
    };
    ...
    if (localStorage.activeSheetData) {
        try {
            ...
            if (window.sheetsState && window.sheetsState.styleLayout) {
                window.applySheetStyleLayout(window.sheetsState.styleLayout);
            } else {
                window.applySheetStyleLayout("default");
            }
        } catch(e) {
            window.sheetsState = null;
            window.applySheetStyleLayout("default");
    ...
    function getSelectedCharacterData() {
        ...
        if (val === "active") {
            ...
            return {
                ...
                shortDescription: window.getSectionText("shortDescription") || "",
                ...
                abilities: window.getSectionText("abilities") || "",
                relations: window.getSectionText("relations") || "",
                appearance: window.getSectionText("appearance") || "",
                background: window.getSectionText("background") || "",
                timeline: window.getSectionText("timeline") || "",
    ...
    function loadSheetDataForSelection(val) {
        ...
        // Apply the style layout loaded from sheetsState
        let styleLayout = (window.sheetsState && window.sheetsState.styleLayout) || "default";
        window.applySheetStyleLayout(styleLayout);
        
        // Set selector dropdown value
        let styleSelector = document.getElementById("sheetStyleSelector");
        if (styleSelector) {
            styleSelector.value = styleLayout;
        }
    }
    ...
    window.generateSheet = async function() {
        ...
        root.charData = charData;
        let prompt = root.prompts.characterSheetPage.sheetGeneration.instruction.evaluateItem;
        ...
        try {
            ...
            if (parsed) {
                ...
                parsed.styleLayout = (window.sheetsState && window.sheetsState.styleLayout) || "default";
    ...
    window.exportSheetHTML = async function() {
        if (!window.sheetsState) {
            alert("Please generate a character sheet first!");
            return;
        }
        ...
        let theme = window.sheetsState.theme || "emerald";
        let styleLayout = window.sheetsState.styleLayout || "default";
        ...
        try {
            ...
            console.warn("Could not fetch style layout rules, using fallback:", e);
            cssRules = `
                body {
...
<body>
    <div class="sheet-wrapper" style="width: 100%; max-width: 900px; margin: 0 auto; display: flex; flex-direction: column; gap: 1.5rem;">
